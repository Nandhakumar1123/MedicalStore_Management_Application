...
def login(request: Request, response: Response, username: str = Form(...), password: str = Form(...)):
    try:
        ...
        if not user:
            return templates.TemplateResponse(request, "login.html", {"request": request, "error": "User not found"})
        ...
        return templates.TemplateResponse(request, "login.html", {"request": request, "error": "Login error. Try again."})
...
def render_medicines(request: Request):
    return templates.TemplateResponse(request, "medicines.html", {"request": request})

@app.get("/api/medicines")
def get_medicines():
    try:
        cur = pg_conn.cursor()
        # Return all medicines (including expired) so the UI can mark expired items
        cur.execute("""
        ...
        for row in data:
            ...
            if expiry is not None:
                try:
                    expiring_soon = (expiry - today).days <= 10
                except Exception:
                    expiring_soon = False
...
def render_add_medicine_form(request: Request):
    return templates.TemplateResponse(request, "add_medicine.html", {"request": request})
...
def add_medicine(med: Medicine):
    ...
    try:
        ...
        # Normalize expiry_date to YYYY-MM-DD when possible (frontend may send formats like '11 - 09 - 2026')
        expiry_val = med.expiry_date
        if expiry_val:
            for fmt in ("%d - %m - %Y", "%d-%m-%Y", "%d/%m/%Y", "%Y-%m-%d"):
                try:
                    expiry_dt = datetime.strptime(expiry_val, fmt).date()
                    expiry_val = expiry_dt
                    break
                except Exception:
                    continue

        added_date_val = med.added_date or datetime.now()
        ...
        if med.id is None:
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
            """, (med.name, expiry_val, med.quantity, med.price, added_date_val))
        else:
            cur.execute("""
                INSERT INTO medicines (id, name, expiry_date, quantity, price, added_date)
                VALUES (%s, %s, %s, %s, %s, %s)
                RETURNING id
            """, (med.id, med.name, expiry_val, med.quantity, med.price, added_date_val))

        # fetchone may return a tuple depending on cursor; handle both
        row = cur.fetchone()
        ...
        # Ensure transaction is rolled back so subsequent requests aren't blocked
        try:
            pg_conn.rollback()
        except Exception:
            pass
...
def show_billing_details(request: Request, billing_id: Optional[int] = None):
    try:
        ...
        return templates.TemplateResponse(request, "billing_details.html", {
            "request": request,
            "billing_data": rows
        })
...
def show_all_billing(request: Request, q: Optional[str] = Query(None)):
    try:
        ...
        return templates.TemplateResponse(request, "billing_history.html", {
            "request": request,
...
def show_medicine_arrivals(request: Request, q: Optional[str] = Query(None)):
    try:
        ...
        return templates.TemplateResponse(request, "medicine_arrivals.html", {
            "request": request,
            "arrivals": arrivals,
            "search_query": q
        })
